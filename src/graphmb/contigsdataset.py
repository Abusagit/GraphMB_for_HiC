...
class ContigsDataset(DGLDataset):
    ...
    def read_seqs(self):
        ...
        with open(os.path.join(self.assembly, self.assembly_name), "r") as f:
            for line in f:
                if line.startswith(">"):
                    contig_name = line[line.find(">") + 1:].strip()
        ...
        with open(os.path.join(self.assembly, self.graph_file), "r") as f:
            for line in f:
                if line.startswith("S"):
                    ...
                    if self.assembly_type == "spades":  # NODE_<ID>_length_<LENGTH>_cov_<COVERAGE>
                        contigid_parts = contigid.split("_")
                        #contigid = "_".join(contigid.split("_")[:2])
                        contiglen = int(contigid_parts[3])
                    else:
                   ...
                   #values[1] = values[1] if self.assembly_type != "spades" else "_".join(values[1].split("_")[:2])
                    #values[3] = values[3] if self.assembly_type != "spades" else "_".join(values[3].split("_")[:2])

                    if values[1] in skipped_contigs or values[3] in skipped_contigs:
                        logger.warning(f"line '{line}' has skipped contig(s):\n {', '.join((c for c in (values[1], values[3]) if c in skipped_contigs))}")
                    ...
                    if values[1] not in self.contig_names or values[3] not in self.contig_names:
                        logger.warning(
                            f"line '{line}' has contig(s) without names\n {', '.join((c for c in (values[1], values[3]) if c not in self.contig_names))}")
                        continue
                    ...
                    if len(values) > 6:
                        rc = float(values[6].split(":")[-1])  # as we have weighted HiC links
                    else:
                        rc = 1  # this will never be used in Hi-C contact map
