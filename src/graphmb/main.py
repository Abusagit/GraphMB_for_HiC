...
def main():
    ...
    # input files
    parser.add_argument("--assembly", type=str, help="Assembly base path", required=False, default="./")
    ...
    # parser.add_argument("--for_amber", help="Make output with prefix for AMBER", action="store_true")
    ...
    parser.add_argument("--latent", type=str, help="Preprocessed latent features for contigs", default='')
    ...
    dataset.nodes_kmer = torch.FloatTensor(stats.zscore(dataset.nodes_kmer))
    ...
    if args.depth is not None and os.path.isfile(os.path.join(args.assembly, args.depth)):
        ...
        if len(dataset.nodes_depths[0]) > 1:  # normalize depths
            dataset.nodes_depths = stats.zscore(dataset.nodes_depths)
    ...
    if args.labels:
        ...
        with open(args.labels, "r") as f:
            for line in f:
                ...
                if args.labels.endswith(".csv"):
                    values = line.strip().split(",")
                elif args.labels.endswith(".tsv"):
                    TSV = True  # amber format
                    if line.startswith("@"):
                        if len(prefix) < 2:
                            prefix.append(line)
                        continue
        ...
        try:
            ...
            print("homophily test isn`t correct - couldn`t process labels")
    ...
    if "cluster" in args.post or "kmeans" in args.post:
        ...
        if "contig2bin" in args.post:
            ...
            with open(args.outdir + f"/{args.outname}_best_contig2bin.tsv", "w") as f:
                f.write(prefix)
                f.write("@@SEQUENCEID\tBINID\t_LENGTH\n")
                for c in best_contig_to_bin:
                    cluster_for_contig_c = best_contig_to_bin[c]
                    contig_length = contig_lens[c]
                    f.write(f"{str(c)}\t{str(cluster_for_contig_c)}\t{int(contig_length * 1_000_000)}\n")

            last_contig_to_bin = {}
            for bin_ in last_cluster_to_contig:
                for contig in last_cluster_to_contig[bin_]:
                    last_contig_to_bin[contig] = bin_
    ...
    if "draw" in args.post:
        ...
        for i in range(10, 50):
            # without_largest_comp = [item for sublist in connected_comp[10:110] for item in sublist if len(sublist)
            # > 2]
